...
const columns: DataTableColumnDef<Opportunity>[] = [
  ...
  },
  {
    accessorKey: "stage",
    header: "Etapa",
    cell: ({ getValue }) => <StatusBadge status={getValue<Opportunity["stage"]>()} config={OPPORTUNITY_STAGE_CONFIG} />,
  },
  {
    accessorKey: "status",
    header: "Estado",
        ...
        <ActionMenu
          items={[
            ...
            },
            {
              ...
              onSelect: () =>
                (table.options.meta as { requestDelete: (row: Opportunity) => void }).requestDelete(row.original),
            },
          ]}
        />
      </div>
